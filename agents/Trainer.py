...
class Trainer(object):
    ...
    def run_games_for_agent(self, agent_number, agent_class, eval= False, checkpoint= False):
        ...
        for run in range(self.config.runs_per_agent):
            ...
            if not eval:
                ...
                game_scores, rolling_scores, time_taken = agent.run_n_episodes_eval()
    ...
    # def load_model(self, agent_number, agent_class, path_actor, path_critic):
    #     agent_name = agent_class.agent_name
    #     agent_config = copy.deepcopy(self.config)
    #
    #     if self.environment_has_changeable_goals(
    #             agent_config.environment) and self.agent_cant_handle_changeable_goals_without_flattening(agent_name):
    #         print("Flattening changeable-goal environment for agent {}".format(agent_name))
    #         agent_config.environment = gym.wrappers.FlattenDictWrapper(agent_config.environment,
    #                                                                    dict_keys=["observation", "desired_goal"])
    #
